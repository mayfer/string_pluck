...
function stringSubCanvas(waves_canvas, wave, base_freq, wave_height, spacer) {
    ...
    this.getPlotCoordinates = function(time_diff) {
        ...
        var normalized_amplitude = Math.sin(this.step + this.wave.phase);
        var current_amplitude = 3 * normalized_amplitude * volume_envelope_amplitude * this.wave_halfheight;
        var x = 0;
        var y = 0;
        var points = [];
...
function superposedStringCanvas(waves_canvas, strings, wave_height) {
    ...
    this.updateAvgY = function(i, y) {        
        if(this.avgYs === undefined) {
            this.avgYs = [];
            this.avgYcounts = [];
        }

        if(this.avgYs[i] === undefined) {
            this.avgYs[i] = 0;
            this.avgYcounts[i] = 0;
        }

        this.avgYs[i] += y;
        this.avgYcounts[i] += 800
    }
    this.draw = function(time_diff) {
        ...
        let string_strethced_length = 0;

        const prev_coords = {x: this.string_position.x, y: this.string_position.y};
        ...
        for(var i = 0; i <= this.num_steps; i++) {
            ...
            // distance from prev coords to current
            const dx = coords.x - prev_coords.x;
            const dy = coords.y - prev_coords.y;
            const distance = Math.sqrt(dx*dx + dy*dy);

            string_strethced_length += distance;

            prev_coords.x = coords.x;
            prev_coords.y = coords.y;
        }

        // take avg of all frequencies
        let total_amplitude = 0;
        let num_samples = 0;
        let total_freq = 0;
        for(var j = 0; j < this.strings.length; j++) {
            for(var i = 0; i < this.strings[j].current_plot_coordinates.length; i++) {
                total_amplitude += this.strings[j].current_plot_coordinates[i].normalized_amplitude;
            }
            num_samples += this.strings[j].current_plot_coordinates.length;
            total_freq += this.strings[j].wave.freq;
        }


        // iterate over each string and modify frequencies based on stretched string length
        for(var j = 0; j < this.strings.length; j++) {
            this.strings[j].wave.freq = this.strings[j].wave.freq + Math.pow(total_amplitude/1000, 1)
            // this.strings[j].wave.gain += (total_freq/1000) / 1000;
        }
