...
function stringSubCanvas(waves_canvas, wave, base_freq, wave_height, spacer) {
    ...
    this.getPlotValues = function(time_diff) {
        ...
        var current_amplitude = 3 * Math.sin(this.step + this.wave.phase) * volume_envelope_amplitude;
        var x = 0;
        var y = 0;
        var points = [];
...
function superposedStringCanvas(waves_canvas, strings, wave_height, wave_position, drawWidth) {
    this.waves_canvas = waves_canvas;
    ...
    this.string_width = drawWidth || this.context.width;
    this.string_position = {x: 0, y: this.context.height * wave_position};
    ...
    this.getPlotValues = function(time_diff) {
        ...
        for(var i = 0; i < this.strings.length; i++) {
            var current_coords = this.strings[i].getPlotValues(time_diff);
            // x is same for all anyways
            vals.push(current_coords[i].y);
        }
        for(var i = 0; i < vals.length; i++) {
            vals[i] /= this.strings.length;
        }
        return vals;
    }
    ...
    this.draw = function(time_diff) {
        ...
        for(var i = 0; i <= num_steps; i++) {
            ...
            if(this.pluck_coordinates) {
                let pluckY = this.getPluckY(time_diff, this.pluck_coordinates, num_steps, i);
...
function stringFFT(points) {
    let freqs = {};
    for(var freq=110; freq<=110*50; freq+= 110) {
        let resonance = 0;
