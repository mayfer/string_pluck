...
class StringProcessor extends AudioWorkletProcessor {
    constructor(...args) {
        ...
        this.port.onmessage = (e) => {
            if(!this.loaded) {
                let mod = e.data;
                loader.instantiate(mod).then((wasmModule) => {
                    const { sound, __getArray } = wasmModule.exports
    ...
    process(inputs, outputs, parameters) {
        let __getArray = this.__getArray
        let channels = outputs[0];
        let buffer_size = channels[0].length;
        if(this.sampleRate && __getArray) {
            //console.log(this.sampleRate, buffer_size)
            let arr = this.__getArray(this.sound(this.sampleRate, buffer_size, 70, this.counter));
            for(let i = 0; i < buffer_size; i++) {
                for(let k = 0; k < channels.length; k++) {
                    channels[k][i] = arr[i]; // Math.sin(Math.PI * 2 * (this.counter + i) * 220 / 44100);
                }
            }
        } else {
            ...
            for(let i = 0; i < buffer_size; i++) {
                for(let k = 0; k < channels.length; k++) {
                    channels[k][i] = 0; //Math.sin(Math.PI * 2 * (this.counter + i) * 220 / 44100);
                }
            }
        }
        this.counter += buffer_size;
        return true
    }
}
